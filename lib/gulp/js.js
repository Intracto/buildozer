...
async function js() {
  ...
  const configurations = await configs.then(configurations => {
    for (const config of configurations) {
      jsTasks = [
        ...jsTasks,
        ...config.js.map(({src, dest}) => {
          return jsCompile({src, dest, cwd: config.cwd});
        })
      ];
    }
  ...
  await Promise.all(jsTasks).then(() => {
    for (const config of configurations) {
      jsConcatTasks = [
        ...jsConcatTasks,
        ...config['js-concat'].map(({src, dest, name}) => {
          return jsConcat({src, dest, name});
        })
      ];
    }
  });
    ...
    (async resolve => { // eslint-disable-line no-async-promise-executor
      ...
      if (eslintConfig) {
        stream = stream.pipe(eslint({
          ...
          .pipe(gulpIf(file => file.eslint?.fixed, gulp.dest(file => file._base)))
          .pipe(eslint.format());
      }
