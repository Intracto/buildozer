...
function cssWatchPaths(paths, cwd) {
  const dirs = [cwd];
  ...
  dirs.forEach(dir => {
    paths.push(path.join(dir, '.stylelintrc'));
    paths.push(path.join(dir, '.stylelintrc.json'));
    paths.push(path.join(dir, '.stylelintrc.yaml'));
    paths.push(path.join(dir, '.stylelintrc.yml'));
    paths.push(path.join(dir, '.stylelintrc.js'));
    paths.push(path.join(dir, 'stylelintrc.config.js'));
  });
  ...
  ['scss', 'js', 'img', 'js-concat', 'svg-sprite', 'copy'].forEach(key => {
    if (config[key]) {
      config[key] = config[key].map(val => {
        const src = path.join(cwd, config.src_base_path, val.src);
        const dest = path.join(cwd, config.dest_base_path, val.dest);
        let watch = [src];

        if (key === 'scss') {
          watch = cssWatchPaths(watch, cwd);
        }
