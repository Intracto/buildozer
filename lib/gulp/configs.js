...
function watchConfigPaths(paths, name, cwd) {
  ...
  for (const dir of dirs) {
    paths.push(
      path.join(dir, `.${name}rc`),
      path.join(dir, `.${name}rc.json`),
      path.join(dir, `.${name}rc.yaml`),
      path.join(dir, `.${name}rc.yml`),
      path.join(dir, `.${name}rc.js`),
      path.join(dir, `${name}.config.js`)
    );
  }
...
function processConfig(config, cwd) {
  ...
  for (const key of ['css', 'js', 'img', 'js-concat', 'svg-sprite', 'copy']) {
    if (config[key]) {
...
async function configs() {
  ...
  for (const arg of process.argv) {
    if (arg.startsWith('--cwd=')) {
      cwd = path.resolve(process.env.INIT_CWD, arg.slice(6));
    }
  }
  ...
  const config = [processConfig({...baseConfig, ...getConfig(cwd)}, cwd)];
