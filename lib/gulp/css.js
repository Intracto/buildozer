const sourcemaps = require('gulp-sourcemaps');
const autoprefixer = require('autoprefixer');
const rfs = require('rfs');
const cssnano = require('cssnano');
const postcss = require('gulp-postcss');
const sass = require('gulp-sass');
const gulp = require('gulp');
const plumber = require('gulp-plumber');
const config = require('./config.js');

// CSS task
function css(cb) {
  config.scss.forEach(scss => {
    cssCompile(config.src_base_path + scss.src, config.dest_base_path + scss.dest, true);
  });
  cb();
}

function cssCompile(src, dest, minified, browserSync = false) {
  let stream = gulp
    .src(src);

  // Load custom PostCSS plugins defined by user
  let postCSSPlugins = config.postcss.map(plugin => {
    if (plugin.name) return require(plugin.name)(plugin.options);
  }).filter(item => !!item);

  // Prevent errors from aborting task when files are being watched
  if (process.argv.includes('watch')) {
    stream = stream.pipe(plumber());
  }

  if (minified) {
    stream = stream
      .pipe(sass({outputStyle: 'compressed'}))
      .pipe(postcss([rfs(), autoprefixer(), cssnano(), ...postCSSPlugins]));
  } else {
    stream = stream
      .pipe(sourcemaps.init())
      .pipe(sass({outputStyle: 'expanded'}))
      .pipe(postcss([rfs(), autoprefixer()], ...postCSSPlugins))
      .pipe(sourcemaps.write('.'));
  }

  stream = stream.pipe(gulp.dest(dest));

  return browserSync === false ? stream : stream.pipe(browserSync.stream({match: '**/*.css'}));
}

module.exports = {css, cssCompile};
