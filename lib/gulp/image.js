...
async function img() {
  ...
  await configs.then(configurations => {
    for (const config of configurations) {
      ...
      spriteTasks = [
        ...spriteTasks,
        ...config['svg-sprite'].map(({src, dest, name}) => {
  ...
  return Promise.all([...imgTasks, ...spriteTasks]);
}
  ...
  return new Promise(
    (resolve => {
      ...
      if (browserSync !== false) {
        stream.pipe(browserSync.stream({match: `**/*.${extension}`}));
      }
...
function svgSprite({src, dest, name, browserSync = false}) {
  return new Promise(
    (resolve => {
      ...
      if (browserSync !== false) {
        stream.pipe(browserSync.stream({match: '**/*.svg'}));
      }
